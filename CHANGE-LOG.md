...
| 2026-08-11 | On-screen version number at the bottom of the app, so a live build can be told apart from a preview at a glance. Not in `03-SPEC.md`; user triaged directly in chat rather than via a filed issue — confirmed to build next session, starting at `0.0.6` (ahead of Slice 7), small enough to go straight to a PR like `wire-list-rename-remove-ui` did. | UI (app shell) | queued — see HANDOFF.md |
